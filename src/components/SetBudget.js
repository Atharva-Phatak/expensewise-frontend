...
export default function SetBudget(props) {
  ...
  return (
    <div className="grid grid-cols-5 bg-black text-white h-3/4 font-lexend">
      <div className="col-span-3 bg-black h-fit p-8 py-14 rounded-md">
        <h1 className="font-bold text-xl mt-3">Set Budget</h1>
        <div className="mt-4">
          <label className="w-fit">Enter the Budget Amount</label>
        </div>

        <div className="flex bg-black w-fit mt-4 p-2 rounded border-rose-600 border-2">
          ...
          <input
            ...
            type="number"
            placeholder=""
            className="setbuget-input bg-black ml-4 outline-none "
          ></input>
        </div>
        <span className="text-sm text-red-500">{error.msg}</span>

        <div className=" mt-20 border-rose-600 border-2 w-fit rounded-md">
          {isLoading ? (
            <ReactLoading
              type="bubbles"
              color="#F5A302"
